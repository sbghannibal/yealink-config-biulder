...
try {
    ...
    // Check if token was already used (optional - allow multiple downloads within validity period)
    // Uncomment to enforce single-use tokens:
    // if ($token_data['used_at']) {
    //     send_error(403, 'Token already used');
    // }
    ...
    // Generate config
    $config_version_id = $token_data['config_version_id'];
    ...
    if ($device_id) {
        ...
        $content = apply_variables_to_content($token_data['config_content'], $variables);
