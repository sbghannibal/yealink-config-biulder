...
<div class="card" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; margin-bottom: 20px;">
    <h3 style="color: white; margin-top: 0;">📊 Last Cleanup Summary</h3>
    
    <?php if ($last_cleanup): ?>
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px;">
            <div style="padding: 16px; background: rgba(255,255,255,0.1); border-radius: 8px;">
                <small style="opacity: 0.8; display: block;">Last Cleanup</small>
                ...
                <small style="opacity: 0.8; display: block; margin-top: 4px;">
                    ...
                    echo $days_ago === 0 ? '🟢 Today' : '📅 ' . $days_ago . ' day(s) ago';
                    ?>
                </small>
            </div>
            
            <div style="padding: 16px; background: rgba(255,255,255,0.1); border-radius: 8px;">
                <small style="opacity: 0.8; display: block;">Cleaned By</small>
                <strong style="font-size: 18px; display: block; margin-top: 8px;">
            ...
            <div style="padding: 16px; background: rgba(255,255,255,0.1); border-radius: 8px;">
                <small style="opacity: 0.8; display: block;">Last Deleted</small>
            ...
            <div style="padding: 16px; background: rgba(255,255,255,0.1); border-radius: 8px;">
                <small style="opacity: 0.8; display: block;">Total Deleted (All Time)</small>
                <strong style="font-size: 18px; display: block; margin-top: 8px;">
    ...
    <?php else: ?>
        <p style="margin: 0;">⏳ No cleanup history yet. Run a cleanup below.</p>
    <?php endif; ?>
</div>
...
<div class="card">
    ...
    <p>Remove configuration versions that are older than the selected period and are not currently active.</p>
    ...
    <form method="post" style="display: flex; gap: 12px; margin: 16px 0; flex-wrap: wrap; align-items: flex-end;">
            ...
            <select name="cleanup_period" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 14px;">
                <?php foreach ($cleanup_periods as $key => $period): ?>
                    <option value="<?php echo htmlspecialchars($key); ?>" <?php echo $selected_period === $key ? 'selected' : ''; ?>>
                        <?php echo htmlspecialchars($period['label']); ?> (<?php echo $period['days']; ?> days)
                    </option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <button type="submit" name="action" value="cleanup" class="btn" style="background: #dc3545; padding: 10px 24px; font-weight: 600;" onclick="return confirm('⚠️ Run cleanup? This will delete configs older than the selected period.');\"
>
            🗑️ Cleanup Now
        </button>
    </form>
    ...
    <div style="background: #f0f8ff; border-left: 4px solid #007bff; padding: 12px; border-radius: 4px; margin-top: 12px;">
        <small style="color: #004085;">
            <strong>💡 Tip:</strong> Only inactive configs are deleted. Active configs are always protected.
        </small>
    </div>
</div>
...
<?php if (!empty($preview)): ?>
    <div class="card">
        <h3>👁️ Preview: <?php echo count($preview); ?> config(s) to delete</h3>
        <p style="color: #666; font-size: 13px;">Configs older than <strong><?php echo htmlspecialchars($cleanup_periods[$selected_period]['label']); ?></strong> (not active)</p>
                        ...
                        <tr style="border-bottom: 1px solid #eee;">
                            ...
                            <td style="padding: 12px;">
                                <span style="background: #ffc107; color: #333; padding: 4px 8px; border-radius: 3px; font-size: 12px; font-weight: bold;">
                                    <?php echo floor((time() - strtotime($cfg['created_at'])) / 86400); ?> days old
                                </span>
                            </td>
                        </tr>
                    <?php endforeach; ?>
...
<div class="card">
    <h3>📋 Cleanup History (Last 5)</h3>
    <?php if (empty($cleanup_log)): ?>
        <p style="color: #999; text-align: center; padding: 20px;">No cleanup history yet.</p>
    <?php else: ?>
        <div style="overflow-x: auto;">
            <table>
                <thead>
                    <tr style="background: #f5f5f5; border-bottom: 2px solid #ddd;">
                        ...
                        <th style="padding: 12px; text-align: left;">Deleted</th>
                        <th style="padding: 12px; text-align: left;">Cleaned by</th>
                    </tr>
                </thead>
                <tbody>
                            ...
                            <td style="padding: 12px;">
                                <?php if ((int)$log['deleted_configs'] > 0): ?>
                                    <span style="background: #d4edda; color: #155724; padding: 4px 8px; border-radius: 3px; font-size: 12px; font-weight: 600;">
                                        ✓ <?php echo (int)$log['deleted_configs']; ?> deleted
                                    </span>
                                <?php else: ?>
